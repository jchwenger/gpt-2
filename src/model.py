...
def default_hparams():
    return HParams(
        n_vocab=0, n_ctx=1024, n_embd=768, n_head=12, n_layer=12, dtype="float32"
    )
...
def norm(x, scope, *, axis=-1, epsilon=1e-5, hparams=None):
    ...
    with tf.compat.v1.variable_scope(scope, dtype=hparams.dtype):
...
def block(x, scope, *, past, hparams):
    with tf.compat.v1.variable_scope(scope):
        nx = x.shape[-1].value
        a, present = attn(
            norm(x, "ln_1", hparams=hparams), "attn", nx, past=past, hparams=hparams
        )
        x = x + a
...
def model(hparams, X, past=None, scope="model", reuse=tf.compat.v1.AUTO_REUSE):
    if hparams.get("dtype") is None:
        hparams.add_hparam("dtype", tf.float32)
    else:
        if hparams.dtype == "float32":
            hparams.dtype = tf.float32
        elif hparams.dtype == "float16":
            hparams.dtype = tf.float16
        elif hparams.dtype == "bfloat16":
